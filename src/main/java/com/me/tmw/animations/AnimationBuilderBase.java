...
public abstract class AnimationBuilderBase<A extends Animation, T extends AnimationBuilderBase<A, T>> implements AnimationBuilder<A, T> {
    ...
    @Override public EventHandler<ActionEvent> getOnFinished() {
        return onFinished.get();
    }
    @Override public ObjectProperty<EventHandler<ActionEvent>> onFinishedProperty() {
        return onFinished;
    }
    @Override public T setOnFinished(EventHandler<ActionEvent> onFinished) {
    ...
    @Override public ObjectProperty<Duration> delayProperty() {
        return delay;
    }
    @Override public T setDelay(Duration delay) {
        this.delay.set(delay);
        return getThis();
    }

    @Override public boolean isAutoReverse() {
        return autoReverse.get();
    }
    @Override public BooleanProperty autoReverseProperty() {
        return autoReverse;
    }
    @Override public T setAutoReverse(boolean autoReverse) {
        this.autoReverse.set(autoReverse);
        return getThis();
    }

    @Override public int getCycleCount() {
        return cycleCount.get();
    }
    @Override public IntegerProperty cycleCountProperty() {
        return cycleCount;
    }
    @Override public T setCycleCount(int cycleCount) {
    ...
    @Override public DoubleProperty rateProperty() {
        return rate;
    }
    @Override public T setRate(double rate) {
        this.rate.set(rate);
        return getThis();
    }
